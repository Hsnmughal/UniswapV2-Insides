# 🔄 UniswapV2-Insides: Mathematical Foundations

This repository provides a comprehensive exploration of the mathematical foundations behind Uniswap V2's core mechanisms. Each section contains both the final formulas (highlighted in code blocks) and their complete step-by-step derivations.
...
swap fee = F * dx
```
- Only the following portion of the input amount participates in the swap:
```
dx(1-F)
```
...
**Derivation:**
1. Current price ratio: 
```
Price = Y₀/X₀
```
2. To maintain the same price: 
```
(Y₀ + dy)/(X₀ + dx) = Y₀/X₀
```
3. Cross-multiplying: 
...
**Derivation:**
1. The ratio of new to old shares equals the ratio of new to old pool value: 
```
(T + S)/T = L₁/L₀
```
2. Solving for S: 
...
S = T * (L₁ - L₀)/L₀
```
3. The relative increase in liquidity equals the relative amount of tokens added: 
```
(L₁ - L₀)/L₀ = dx/X₀ = dy/Y₀
```
...
**Derivation:**
1. The decrease in liquidity is proportional to shares burned: 
```
(L₀ - L₁)/L₀ = S/T
```
2. This equals the proportional decrease in each token: 
```
S/T = DX/X₀ = DY/Y₀
```

**Key Principle:**
When removing liquidity, the price ratio must remain unchanged:
```
dy/dx = Y₀/X₀
```

This can be derived by setting the price before equal to the price after: 
```
(Y₀ - dy)/(X₀ - dx) = Y₀/X₀
```
...
**Derivation:**
1. The ratio of remaining to original shares equals the ratio of remaining to original liquidity: 
```
(T - S)/T = L₁/L₀
```
2. Solving for withdrawn value: 
```
L₀ - L₁ = L₀ - L₀ * (T - S)/T = L₀ * [1 - (T - S)/T] = L₀ * [S/T]
```
...
**Derivation:**
1. Define cumulative price: 
```
C_j = Σ(i=0 to j-1) Δt_i × p_i
```
   - This is the sum of price × duration products up to time j
2. The sum of price changes from t_k to t_n equals:
```
(C_n - C_k)
```
